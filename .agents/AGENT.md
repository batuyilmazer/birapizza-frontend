...
*Master Debug Strategy:* If debugging is needed, start by forming a hypothesis about the root cause. If your proposed fix doesn't work, add debug instrumentation to the code to gather more signal. Use the logs from that instrumentation to confirm, refute, or refine your hypothesis. Once the bug is resolved, remove all debug instrumentation artifacts from the code.
